 ...
 #    
  ...
  '<details>
     <summary style="
        ...
        border-radius: 4px;
        margin-bottom: 5px;">
        <u>%s <span style="background-color:%s; color:white; padding:2px 6px; border-radius:4px;">%s</span></u>
     </summary>\n\n',
  pillar_ecol_df2$indicator[keep[j]],   # indicator text
  ifelse(is.na(pillar_ecol_df2$score[keep[j]]) | pillar_ecol_df2$score[keep[j]] == "NA", "#EDEDED", unname(flowerPalette[calc_letter_grade(pillar_ecol_df2$score[keep[j]])])), # background color
  ifelse(is.na(pillar_ecol_df2$score[keep[j]]) | pillar_ecol_df2$score[keep[j]] == "NA", " ", pillar_ecol_df2$score[keep[j]])                         # grade to display
))
