...
#JAIM
              ITP$plot[which(ITP$indicator == "Physical conditions (e.g. temperature, salinity, light levels, wind, sea-surface height) within the MR (surface and bottom) and both upstream and downstream, as measured on the AZMP’s line.")] <- "plot_azmp_physical(mpa=MPAs)"
              ...
              for (i in seq_along(ITP$indicator)) {
                ...
                if (itp == "Support productivity objectives for groundfish species of Aboriginal, commercial, and/or recreational importance, particularly NAFO Division 4VW haddock") {
                  ...
                  if (itp == "Physical conditions (e.g. surface temperature) within the MR") {
                    df <- plot_azmp_physical(mpa=MPAs,parameter="temperature", type="surface", dataframe=TRUE)
                    ITP$plot[i] <- 'plot_azmp_physical(mpa=MPAs,parameter="temperature", type="surface")'
                  } else if (itp == "Physical conditions (e.g. bottom temperature) within the MR") {
                    df <- plot_azmp_physical(mpa=MPAs,parameter="temperature", type="bottom", dataframe=TRUE)
                    ITP$plot[i] <- 'plot_azmp_physical(mpa=MPAs,parameter="temperature", type="bottom")'
                  } else if (itp == "Physical conditions (e.g. surface salinity) within the MR") {
                    df <- plot_azmp_physical(mpa=MPAs,parameter="salinity", type="surface", dataframe=TRUE)
                    ITP$plot[i] <- 'plot_azmp_physical(mpa=MPAs,parameter="salinity", type="surface")'
                  } else if (itp == "Physical conditions (e.g. bottom salinity) within the MR") {
                    df <- plot_azmp_physical(mpa=MPAs,parameter="salinity", type="bottom", dataframe=TRUE)
                    ITP$plot[i] <- 'plot_azmp_physical(mpa=MPAs,parameter="salinity", type="bottom")'
                  } else if (itp == "Biological conditions (e.g. surface chlorophyl) within the MR") {
                    df <- plot_azmp_physical(mpa=MPAs,parameter="chlorophyll", type="surface", dataframe=TRUE)
                    ITP$plot[i] <- 'plot_azmp_physical(mpa=MPAs,parameter="chlorophyll", type="surface")'
                  } else if (itp == "Biological conditions (e.g. surface nitrate) within the MR") {
                    df <- plot_azmp_physical(mpa=MPAs,parameter="nitrate", type="surface", dataframe=TRUE)
                    ITP$plot[i] <- 'plot_azmp_physical(mpa=MPAs,parameter="nitrate", type="surface")'
                  } else if (itp == "Biological conditions (e.g. surface phosphate) within the MR") {
                    df <- plot_azmp_physical(mpa=MPAs,parameter="phosphate", type="surface", dataframe=TRUE)
                    ITP$plot[i] <- 'plot_azmp_physical(mpa=MPAs,parameter="phosphate", type="surface")'
                  }
