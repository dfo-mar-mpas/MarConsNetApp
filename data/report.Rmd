---
title: |
  Conservation Area: `r params$mpas`
  Year in Review: `r as.numeric(format(Sys.time(), "%Y")) - 1`
output:
  html_document:
    self_contained: true
    toc: true
...
if (length(com$project_id) != 0) {
  ...
  for (i in seq_along(ps)) {
    ...
    for (j in seq_along(PS$project_id)) {
      keep <- which(data_om$project_id == PS$project_id[j])
      adjusted_ps[[i]]$amount[j] <- sum(data_om$amount[keep] * PS$percent_sites_in_mpa[j])
    }
  }
...
## FINDING NUMBER OF INDICATORS CONTRIBUTING TO THIS AREA
longtermid <- sub(".*PPTID:\\s*", "", unique(proj_ids_samples$project_id))
keepO <- which(objective_tabs$area %in% params$mpas)
S_Objectives <- objective_tabs$objectives[keepO]
number_of_co <-vector("list", length(longtermid))
for (i in seq_along(longtermid)) {
  
  k1 <- which(as.character(pillar_ecol_df2$PPTID) == longtermid[i])
  matched_objectives <- NULL
  
  for (j in seq_along(S_Objectives)) {
    hits <- k1[grepl(S_Objectives[j], pillar_ecol_df2$objectives[k1])]
    if (length(hits) > 0) {
      matched_objectives <- c(matched_objectives, S_Objectives[j])
    }
  }
  
  number_of_co[[i]] <- unique(matched_objectives)
  
  names(number_of_co)[[i]] <-
    unique(pillar_ecol_df2$project_short_title[
      pillar_ecol_df2$PPTID == longtermid[i]
    ])[!is.na(unique(pillar_ecol_df2$project_short_title[
      pillar_ecol_df2$PPTID == longtermid[i]
    ]))]
}
#names(number_of_co_indicators) <- longtermid

## end

## Creature feature
 filtered_creature_feature <- creature_feature |>
      filter(NAME_E == params$mpas) |>
      arrange(desc(n)) |>
      dplyr::select(commonname, image_column, scientificName, n, kingdom, phylum, class, years)
 
 
 
 if (any(grepl(as.numeric(format(Sys.time(), "%Y"))-1, filtered_creature_feature$years))) {
yr <- as.numeric(format(Sys.time(), "%Y"))-1

rows <- filtered_creature_feature[grepl(yr, filtered_creature_feature$years), ]

counts <- sub(".*\\((\\d+)\\).*", "\\1", rows$years)

parts <- paste(counts, rows$scientificName)

creature_feature_sentence <- paste0(
  "In ", yr, ", there ",
  if(length(parts) == 1) "was " else "were ",
  paste(parts, collapse = " and "),
  " identified based on iNaturalist."
)

} else {
  creature_feature_sentence <- paste0("No species were identified in ", params$mpas, " in ",as.numeric(format(Sys.time(), "%Y"))-1, " according to iNaturalist.")
}
...
div(
  ...
  tags$p(investment),
  tags$p(sample_statement),
  tags$p(creature_feature_sentence),
...
cat("**Key Research and Monitoring Activities**")
cat("<br>")
cat("<br>")
...
cat("<br>")
cat("<br>")
cat("**New Publications**")
cat("<br>")
cat("<br>")
deliv_keep <- which(data_deliverables$project_id %in% p_subset & data_deliverables$year == last_year) # Only projects for this area / last year

if (length(deliv_keep > 0)) {
  delv <- data_deliverables[deliv_keep,][c("classification_display", "description")]
...
cat("<br>")
cat("<br>")
csas_keep <- which(grepl(paste(params$mpas, collapse = "|"), data_csas$title)) # params$mpas
if (params$mpas == "Western and Emerald Banks Marine Refuge") {
csas_keep <- c(csas_keep, which(grepl("Emerald Banks", data_csas$title, ignore.case=TRUE)))
  ...
  csas_keep <- c(csas_keep, which(grepl("Musquash", data_csas$title, ignore.case=TRUE)))
}
...
cat("<br>")
cat("<br>")
cat("**Long term studies**")
cat("<br>")
cat("<br>")
co_statements <- sapply(names(number_of_co), function(proj) {
  
  objs <- number_of_co[[proj]]
  
  if (is.null(objs) || length(objs) == 0) {
    paste0(proj, " did not contribute to any site conservation objectives for this area.")
    
  } else if (length(objs) == 1) {
    paste0(proj, " contributed to the following site conservation objective for this area: ",
           objs, ".")
    
  } else {
    paste0(proj, " contributed to the following site conservation objectives for this area: ",
           paste(objs, collapse = "; "), ".")
  }
})

cat(paste0(co_statements, collapse = "\n\n"))

cat("<br>")
cat("<br>")
cat("**Collaborative Projects**")
cat("<br>")
cat("<br>")
...
```{r, echo = FALSE, comment=NA, results="asis", warning=FALSE}
cat("<br>")
cat(creature_feature_sentence)
cat("<br>")
```

## Results

<h3 style="display:none;">MANAGEMENT EFFECTIVENESS</h3>
<details>
...
```{r, echo=FALSE,comment=NA, results="asis", warning=FALSE}
cat("**Site Conservation Objectives**\n")
cat("<br>")
...
OBJECTIVES_SOWHAT <- vector("list", length(textO))
OBJECTIVES_GAP_INDICATORS <- vector("list", length(textO))

for (i in seq_along(textO)) {
  
cat(sprintf(
  '<h4 style="display:none;">Objective: %s</h4>
   <details>
     <summary style="
        cursor: pointer;
        font-weight: bold;
  ...
  toupper(textO[i]),
  site_color[i],
  ifelse(!(dt_data$Grade[i] == "NA"), dt_data$Grade[i], "  ")
))
  ...
  if (!(length(keep) == 0)) {
    ...
    n_indicators <- length(which(!(is.na(objective_flower$score))))
      ...
      n_indicators_outside <- length(unique(objective_flower$objective[!is.na(objective_flower$adjacent_score)]))
        ...
        sowhat <- paste0(
          "This objective has a score of ", dt_data$Grade[i],
        ...
        OBJECTIVES_SOWHAT[[i]] <- paste0(sowhat, ".", sowhatoutside)
    }
        cat("<br>")
if (!(length(keep) == 0)) {
...
cat("</details>\n\n")  # close the collapsible
  ...
  OBJECTIVES_GAP_INDICATORS[[i]] <- paste0(pillar_ecol_df2$indicator[keep_na], collapse=", ")
...
<h3 style="display:none;">EFFECTIVENESS CONTRIBUTIONS</h3>
<details>
<summary style="font-size:1.5em; font-weight:bold; cursor:pointer;">
...
cat("**Network Conservation Objectives**\n")
cat("<br>")
...
cat(sprintf(
  ...
  toupper(n_objectives[i]),             # hidden heading for TOC
  toupper(n_objectives[i]),             # visible objective text
  grade_colors_network[i],              # background color
  ifelse(!(grades_network[i] == "NA"), grades_network[i], "  ")  # grade text
))
...
<h3 style="display:none;">ECOSYSTEM OVERVIEW</h3>
<details>
<summary style="font-size:1.5em; font-weight:bold; cursor:pointer;">
ECOSYSTEM OVERVIEW
</summary>
...
```{r, echo=FALSE, comment=NA, results="asis", warning=FALSE}
cat("<br>")
cat("The Ecosystem Overview section presents all indicators measured in the selected area, regardless of whether they contribute to specific conservation objectives. ")
cat("<br>")
...
cat("<br>")
        sowhat <- paste0(
          "The ecosystem health has a score of ", calc_letter_grade(weighted.mean(subset_ped$score, subset_ped$weight, na.rm=TRUE)),
          " indicating ", tolower(grade_text), ".",
        ...
        ECOSYSTEM_HEALTH_SOWHAT <- paste0(sowhat," ", sowhat)
...
SOWHATBIN <- vector("list", length(keeper2))
BIN_GAP_INDICATORS <- vector("list", length(keeper2))
...
cat(sprintf(
  ...
  toupper(BINS[i]),          # hidden heading for TOC
  toupper(BINS[i]),          # visible text in the summary
  GRADE_COLOR,               # background color for grade
  ifelse(!(is.na(BIN_GRADE)), BIN_GRADE, "  ")  # grade to display
))
  ...
  if (!(length(keeper2[[i]]$Indicator) == 0)) {
    ...
    grade_text_bin <- grade_description('ecosystem_health')[[calc_letter_grade(weighted.mean(keeper2[[i]]$Grade, keeper2[[i]]$weight, na.rm=TRUE))]]
    n_indicators_bin <- length(which(!(is.na(keeper2[[i]]$Grade))))
    
      years_bin <- str_extract(unique(str_extract(keeper2[[i]]$quality, "\\(([^)]*)\\)")), "(?<=-)[0-9]+")
      years_num_bin <- as.numeric(years_bin)
      latest_year_bin <- ifelse(length(years_num_bin) > 0 && any(!is.na(years_num_bin)),max(years_num_bin, na.rm = TRUE),NA)
      
      weighted_outside_grade_bin <- calc_letter_grade(
          weighted.mean(keeper2[[i]]$adjacent_score,keeper2[[i]]$weight, na.rm = TRUE)
        )
      
              n_indicators_outside_bin <- length(unique(keeper2[[i]]$Indicator[!is.na(keeper2[[i]]$adjacent_score)]))

        
        

        sowhat_bin <- paste0(
          "This objective has a score of ", calc_letter_grade(weighted.mean(keeper2[[i]]$Grade, keeper2[[i]]$weight, na.rm=TRUE))," indicating ", tolower(grade_text_bin), ".",

          if (n_indicators_bin < 4) {
            paste0(" It should be noted that this assessment is only based on the score of ", n_indicators, " unique indicators and further research is recommended.")
          } else {
            paste0(" This assessment is based on ", n_indicators_bin, " indicators.")
          },

          if (!is.na(latest_year_bin)) {
            paste0(" The latest data we have that supports this objective is as of ", latest_year_bin, ".")
          } else {
            " "
          })
        
        
          if (!(all(is.na(keeper2[[i]]$adjacent_score)))) {
            sowhatoutside_bin <- paste0(" For context, outside of the protected area has scored a ", weighted_outside_grade_bin, ' indicating ',
                                    tolower(grade_description('objective')[[weighted_outside_grade_bin]]), ". This outside assessment is based on
                                    ",n_indicators_outside_bin, " indicators.")
          } else {
            sowhatoutside_bin <- ""
          }
        ...
        cat(sowhat_bin)
        cat("<br>")
        cat(sowhatoutside_bin)
        cat("<br>")
        
        SOWHATBIN[[i]] <- paste0(sowhat_bin, " ", sowhatoutside_bin)
    
    
    ## END SO WHAT
    
    # SUMMARY TABLE
       cat('<h5 style="text-align:left; margin-bottom:10px;">Table: Ecological Overview Summary</h5>\n')
...
# END SUMMARY TABLE
  ...
  "\n\n"
      ...
      ## Adding indicator so-what for ecological overview
...
if (!(weighted_grade_ind_eo == "NA")) {
  ...
  cat("<br>")
    BIN_GAP_INDICATORS[[i]] <- paste0(bin_na_indicators, collapse=", ")
...
names(SOWHATBIN) <- names(keeper2)
```
</details>
<h3 style="display:none;">THREATS</h3>
<details>
<summary style="font-size:1.5em; font-weight:bold; cursor:pointer;">
THREATS
</summary>
...
cat("<br>")
 ...
 # Preamble text
cat(
  "The Threats section focuses specifically on indicators that measure pressures and threats to the ecosystem. ",
...
if (!(length(ddff_unique$PPTID) == 0)) {
  ...
  cat('<table border="1" style="border-collapse:collapse; width:100%;">\n')
cat('<thead><tr>',
    paste0('<th>', c("GROUPING", "BIN", "INDICATOR", "SOURCE", "SCORE", "READINESS", "QUALITY_STATEMENT", "COST", "GRADE"), '</th>', collapse=""),
    '</tr></thead>\n')
cat('<tbody>\n')
...
for (i in seq_len(nrow(ddff_threats))) {
  ...
  cat('<td style="background-color:white;">', ddff_threats$QUALITY_STATEMENT[i], '</td>')
...
if (length(com$project_id) != 0) {
  ...
  om_in_site <- data_om[which(data_om$project_id %in% unique(com$project_id)),]
...
if (length(com$project_id) != 0) {
dataSPA_plot <- dataSPA::plotSPA(data_om = om_in_site, which = "omBar")
...
cat("From a management perspective,")
cat("<br>")
for (i in seq_along(OBJECTIVES_SOWHAT)) {
  cat("**", names(OBJECTIVES_SOWHAT)[i], "**\n\n", sep = "")
  
  val <- OBJECTIVES_SOWHAT[[i]]
  
  if (is.null(val) || identical(val, "") || length(val) == 0) {
    cat("No indicators measured for this objective. Further research recommended.\n\n")
    cat("The following indicators were identified as indicators that will inform this objective that we don't yet have data for. They should be considered for future research: ")
    cat(OBJECTIVES_GAP_INDICATORS[[i]])
  } else {
    cat(val, "\n\n")
    cat("The following indicators were identified as indicators that will inform this objective that we don't yet have data for. They should be considered for future research: ")
    cat(OBJECTIVES_GAP_INDICATORS[[i]])
...
cat("<br>")
cat("The establishment of this area has also contributed to knowledge about network objectives, ecosystem-based management objectives, and global biodiversity targets.")
cat("<br>")
cat("<br>")

cat("From a ecosystem health perspective,")
cat("<br>")

cat(ECOSYSTEM_HEALTH_SOWHAT)
cat("<br>")
cat("<br>")
...
for (i in seq_along(SOWHATBIN)) {
  cat("<br>")
  cat("**", toupper(names(SOWHATBIN))[i], "**\n\n", sep = "")
  
  val <- SOWHATBIN[[i]]
  
  if (is.null(val) || identical(val, "") || length(val) == 0) {
    cat("No indicators measured for this objective. Further research recommended.\n\n")
    cat("The following indicators were identified as indicators that will inform this indicator bin that we don't yet have data for. They should be considered for future research: ")
    cat(BIN_GAP_INDICATORS[[i]])
  } else {
    cat(val, "\n\n")
    cat("The following indicators were identified as indicators that will inform this indicaotor bin that we don't yet have data for. They should be considered for future research: ")
    cat(BIN_GAP_INDICATORS[[i]])
...
if (any(p_subset %in% data_collaborations$project_id)) {
  good_col <- which(data_collaborations$project_id %in% p_subset)
  collab_subset <- data_collaborations[good_col,]
  ...
  if (!(length(year_keep) == 0)) {
   ...
   organizations <- unique(recent_col$Organization)
    ...
    cat(paste0("The following data_collaborations/partnerships were identified for ", last_year, " for projects that contributed to this site:"))
    cat("<br>")

    print(knitr::kable(rc, caption = "Recent Collaborations and Partnerships",row.names=FALSE))
    cat("\n\n")
  } else {
  cat(paste0("In year ", last_year, " there were no identified data_collaborations/ partnerships. "))
  cat("<br>")
  ...
  if (!(length(which((!(1:length(collab_subset$project_id)%in% year_keep))))) == 0) {
    ...
    cat("The following past partnerships/data_collaborations have been identified and contributed to this site:")
    cat("<br>")
    ...
    cat("There are no partnerships/data_collaborations identified from previous years.")
    cat("<br>")
    ...
    cat("There have been no identified data_collaborations/ partnerships for this area using the Project Planning Tool.")
    cat("<br>")
