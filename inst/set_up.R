...
if (
  ...
  unique_table <- pillar_ecol_df %>%
    ...
    left_join(
      pillar_ecol_df %>%
        ...
        summarize(
          target_names = paste(unique(target_name), collapse = ", "),
          .groups = "drop"
        ),
  ...
  unique_table <- unique_table[
    -which(unique_table$indicator %in% MPAs$NAME_E),
  ]
  ...
  for (i in seq_along(target_files)) {
    ...
    if (any(which(trimws(first_lines) == "tar_target("))) {
      ...
      second_lines <- sapply(chunks, `[`, 2)[which(
        trimws(first_lines) == "tar_target("
      )]
    ...
    target_names[[i]] <- data.frame(
      indicator = sub(".*tar_target\\(([^,]+),.*", "\\1", first_lines)
    )
  ...
  for (i in seq_along(unique_table$target_names)) {
    ...
    unique_table$code[i] <- paste0(
      targets_and_code_ordered$code,
      collapse = "\n\n"
    )
  ...
  save(
    unique_table,
    file = paste0(dirname(path_to_store()), '/data/unique_table_cost.rda')
  )
}
