indicator_targets <- list(
  ...
  tar_target(
    name = ind_benthic_characteristics_diversity,
    {
      ...
      x <- process_indicator(
        data = data[data$class == mappedclass, ],
        readiness = "Ready",
    ...
    tar_map(
      values = tibble::tibble(
        trophic = read_excel(
          ...
          #TODO check that removing "Suspension feeder" still valid, because none found in data_edna_data
          mutate(
            trophic_group = gsub("Suspension feeder", "Unknown", trophic_group)
          ) |>
          pull(trophic_group) |>
          unique() |>
          sort()
...
",
  ...
  tar_target(name = ind_distinctive_benthic_characteristics_kelp, command = {
    ...
    x <- process_indicator(
      ...
      PPTID = c(1633, 2576),
      source = "OneDrive",
      project_short_title = c(
        'Development and application of high throughput community monitoring',
        'Predicting and assessing interannual change in kelp forest habitat'
      ),
      areas = mpas,
  ...
  tar_target(
    name = ind_distinctive_benthic_characteristics_macroalgae,
    command = {
  ...
  tar_target(name = ind_nonindigenous_rel_indigenous, command = {
    ...
    x <- process_indicator(
      data = data[
        which(
          data$scientificName %in%
            c("Membranipora membranacea", "Fucus serratus")
        ),
      ],
      readiness = "Ready",
  ...
  tar_target(name = ind_invasive, command = {
    ...
    x <- process_indicator(
      data = data[
        which(data$scientificName %in% c("Membranipora membranacea")),
      ],
      readiness = "Ready",
      indicator_var_name = "measurementValue",
  ...
  tar_target(name = ind_species_per_trophic_epibenthic_communities, command = {
    message(class(data_epibenthic_communities_biological))
    trophic_levels <- read_excel(paste0(
      dirname(path_to_store()),
      "/data/AI_trophic_groups.xlsx"
    ))
    ...
    for (i in seq_along(unique(data_epibenthic_communities_biological$class))) {
      data_epibenthic_communities_biological$ai_trophic_level[which(
        data_epibenthic_communities_biological$class ==
          unique(data_epibenthic_communities_biological$class)[i]
      )] <- trophic_levels$trophic_group[which(
        trophic_levels$class ==
          unique(data_epibenthic_communities_biological$class)[i]
      )]
    }
    ...
    mpas <- MPAs %>%
      st_filter(data_epibenthic_communities_biological[
        which(
          data_epibenthic_communities_biological$ai_trophic_level == 'Predator'
        ),
      ]) %>%
      filter(NAME_E != "Non_Conservation_Area")
...
",
  ...
  tar_target(ind_otn_proportion_tags_detected_in_multiple_mpas, command = {
    ...
    for (i in seq_along(unique(x$areaID))) {
      ...
      if (length(keep) > 0) {
        ...
        tester_data$year_of_data_collection <- as.numeric(substr(
          tester_data$eventDate,
          1,
          4
        ))
  ...
  tar_target(name = ind_musquash_coliform, command = {
    data <- data_musquash_coliform |>
      mutate(
        year_of_data_collection = as.numeric(year),
        year_of_publication = 2025
      ) |>
      dplyr::select(
        latitude,
  ...
  tar_target(ind_temperature_inside_outside, command = {
    control_polygons
    MPAs
    data <- data_azmp_Discrete_Occupations_Sections |>
      dplyr::select(longitude, latitude, year, depth, temperature)
  ...
  tar_target(
    name = ind_designtarget,
    {
      ...
      if (
        grepl("BiophysicalUnits", layername) |
          grepl("GeomorphicUnits", layername)
      ) {
        cp <- st_read(data_designtargets_gdb, sub("_.*", "", layername)) |>
          st_make_valid() |>
          left_join(data_designtargets_lookup, by = "NAME") |>
        ...
        grepl("NaturalDisturbance", layername) |
          grepl("ScopeForGrowth", layername)
      ) {
        cp <- st_read(data_designtargets_gdb, sub("_.*", "", layername)) |>
          st_make_valid() |>
      ...
      x <- process_indicator(
        ...
        type = case_when(
          # ---- MODEL ----
          str_detect(plainname, "SDM") ~ "model",
          str_detect(plainname, "KDE") ~ "model",
          str_detect(
            description,
            regex("Modelled", ignore_case = TRUE)
          ) ~ "model",
          str_detect(
            description,
            regex("Classification system", ignore_case = TRUE)
          ) ~ "model",

          # ---- IN SITU ----
          str_detect(
            description,
            regex("survey data", ignore_case = TRUE)
          ) ~ "in situ",
          str_detect(type, regex("landing", ignore_case = TRUE)) ~ "in situ",

          # ---- EXPERT OPINION ----
          str_detect(
            description,
            regex("Formally declared", ignore_case = TRUE)
          ) ~ "expert opinion",
        ...
        bin_rationale = "FIXME",
        theme = case_when(
          type == "Biodiversity hotspot" ~ "Trophic Structure and Function",
          type == "Commercial fishery landing" ~ "Fish and Fishery Resources",
          type ==
            "Natural disturbance classification" ~ "Ocean Structure and Movement",
          TRUE ~ "Benthic Environment"
        ), #TODO this theme is probably not correct?

        objectives = NA,
        readiness = "Ready",
        scale = "region-site",
        SME = "Unknown",
        data_year_of_publication = 2021,
        plot_type = "map"
      )
  ...
  tar_target(
    ind_placeholder_df,
    ind_placeholder(areas = MPAs, readiness = "Unknown")
  ),
...
"
  ...
  tar_target(name = ind_fresh_scars, command = {
    ind_placeholder(
      ind_name = "% of individuals in the Scotian Shelf Northern Bottlenose population showing fresh scars",
      areas = MPAs[which(MPAs$NAME_E == "Gully Marine Protected Area"), ],
      readiness = "Unknown",
      source = NA,
      objectives = c(
