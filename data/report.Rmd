...
image_folder <- file.path(suppressMessages(path_to_store()),"data","plot")
...
com <- cost_of_mpas[which(cost_of_mpas$area == params$mpas & !(round(cost_of_mpas$percent_sites_in_mpa,0) == 0)),]
...
if (string %in% names(Objectives_processed)) {
keepO <- which(names(Objectives_processed) == params$mpas)
...
# cat('<h4 style="text-align:left; margin-bottom:10px;">Table 2: Network Objective Effectiveness Summary</h3>\n')
# cat('<table style="border-collapse: collapse; width:100%; border: 2px solid black; font-size: 1.1em;">\n')
# cat('<tr><th style="border: 2px solid black; padding: 6px;">Objective</th><th style="border: 2px solid black; padding: 6px;">Grade</th></tr>\n')
# 
# for(i in seq_len(nrow(dt_data_network))) {
#   cat('<tr style="background-color:', grade_colors_network[i], ';">')
#   cat('<td style="border: 2px solid black; padding: 6px;">', dt_data_network$Objective[i], '</td>')
#   cat('<td style="border: 2px solid black; padding: 6px;">', dt_data_network$Grade[i], '</td>')
#   cat('</tr>\n')
# }
# 
# cat('</table>')
...
cat(sprintf(
  ...
  grades_network[i]               # grade to display
))
  ...
  for (j in seq_along(keep)) {
    ...
    # FINDING PLOT
      keep1 <- which(grepl(make.names(params$mpas), image_files, ignore.case=TRUE)) # Which are from the correct area
      keep2 <- which(grepl(make.names(pillar_ecol_df$indicator[keep[j]]), image_files, ignore.case=TRUE)) # Which ones have the correct indicator name
...
# cat('<tr><th style="border: 2px solid black; padding: 6px;">Objective</th><th style="border: 2px solid black; padding: 6px;">Grade</th></tr>\n')
# 
# for (i in seq_len(nrow(dt_data))) {
#   cat('<tr style="background-color:', site_color[i], ';">')
#   cat('<td style="border: 2px solid black; padding: 6px;">', dt_data$Objective[i], '</td>')
#   cat('<td style="border: 2px solid black; padding: 6px;">', dt_data$Grade[i], '</td>')
#   cat('</tr>\n')
# }
# 
# cat('</table>\n')
...
cat(sprintf(
  ...
  site_color[i],        # background color
  toupper(textO[i]),       # objective text
  dt_data$Grade[i]               # grade to display
))
...
if (!(length(ddff_unique$PPTID) == 0)) {
  ...
  cat('<h4 style="text-align:left; margin-bottom:10px;">Table 3: Ecological Overview Summary</h3>\n')
...
```{r, echo=FALSE, comment=NA, results='asis'}
cat('<h2>Research and Monitoring Area</h2>\n')
```
...
for (i in seq_along(keeper2)) {
  ...
  for (j in seq_along(keeper2[[i]]$Indicator)) {
    ...
    # FINDING PLOT
      k1 <- which(grepl(make.names(params$mpas), image_files, ignore.case=TRUE)) # Which are from the correct area
