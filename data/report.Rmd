...
if (!(length(ddff_unique$PPTID) == 0)) {
  ...
  for (i in seq_len(nrow(ddff_unique))) {
    # if you want row coloring, define here
    grade <- calc_letter_grade(ddff_unique$SCORE[i])   # your function
    if (!(is.na(grade))) {
      if (!(grade == "NA")) {
    ...
    cat('<td>', ddff_unique$GROUPING[i], '</td>')
    cat('<td>', ddff_unique$BIN[i], '</td>')
    cat('<td>', ddff_unique$INDICATOR[i], '</td>')
    cat('<td>', ddff_unique$SOURCE[i], '</td>')
    cat('<td>', sprintf("%.2f", ddff_unique$SCORE[i]), '</td>')
    cat('<td>', ddff_unique$READINESS[i], '</td>')
    cat('<td>', ddff_unique$QUALITY[i], '</td>')
    cat('<td>', ddff_unique$COST[i], '</td>')
    cat('</tr>\n')
  }
