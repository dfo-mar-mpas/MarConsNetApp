...
table_ped <- table_ped[,c("bin", "indicator", "source", "score", "weight", "PPTID", 'readiness', 'areaID')]
...
if (length(cost_of_mpas2$project_id) != 0) {
  ...
  for (i in seq_along(projects)) {
    ...
    data_om$amount[which(as.numeric(data_om$project_id) == as.numeric(projects[i]))] <- data_om$amount[which(as.numeric(data_om$project_id) == as.numeric(projects[i]))]*percent_sites_in_network
    OMs[[i]] <- data_om[which(as.numeric(data_om$project_id) == as.numeric(projects[i])),]
  }
  network_om <- do.call(rbind, OMs)
  plotly_plot <- try(dataSPA::plotSPA(data_om=network_om, which="omBar"), silent=TRUE)
  ...
  SALs <- list()
   for (i in seq_along(projects)) {
    percent_sites_in_network <- sum(cost_of_mpas2$percent_sites_in_mpa[which(cost_of_mpas2$project_id == projects[i])])
    data_salary$amount_week[which(as.numeric(data_salary$project_id) == as.numeric(projects[i]))] <- data_salary$amount_week[which(as.numeric(data_salary$project_id) == as.numeric(projects[i]))]*percent_sites_in_network
    SALs[[i]] <- data_salary[which(as.numeric(data_salary$project_id) == as.numeric(projects[i])),]
  }
  network_salary <- do.call(rbind, SALs)
  salary_plot <-  try(dataSPA::plotSPA(data_salary=network_salary, which="salaryBar"), silent=TRUE)
...
for (i in seq_along(ps)) {
  ...
  for (j in seq_along(PS$project_id)) {
    keep <- which(data_salary$project_id == PS$project_id[j])
    adjusted_ps_salary[[i]]$amount[j] <- sum(data_salary$amount_week[keep] * PS$percent_sites_in_mpa[j])
  }
}
...
if (length(deliv_keep > 0)) {
  ...
  cat("There has been no identified data_deliverables in the Project Planning Tool for projects supporting this area.")
}
...
csas_keep <- c(csas_keep, which(grepl("Emerald Banks", data_csas$title, ignore.case=TRUE)))
  ...
  csas_keep <- c(csas_keep, which(grepl("Anns", data_csas$title, ignore.case=TRUE)))
