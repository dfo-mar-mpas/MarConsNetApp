...
tar_option_set(packages = basename(pkgs), format = "qs")
...
if (!nzchar(store)) {
  stop("MARCONSNET_TARGETS_PATH is not set!")
}
...
list(
  ...
  tar_target(name = climate, command = {
    x <- list.files(
  ...
  tar_target(
    Outside, #FIXME: this is only for WEBCA at the moment
    st_transform(
      read_sf(system.file(
        "data",
        "WEBCA_10k_85k.shp",
        package = "MarConsNetAnalysis"
      ))$geometry,
      crs = 4326
    )
  ),
  ...
  tar_target(name = Ecological, command = {
    data.frame(
      grouping = rep(
        c("Biodiversity", "Habitat", "Productivity"),
        times = c(3, 5, 3)
      ),
      labels = c(
        "Genetic Diversity",
  ...
  tar_target(name = APPTABS, command = {
    apptabs <- expand.grid(
      ...
      mutate(
        tab = paste0("tab_", 1:length(flower)),
  ...
  tar_target(name = cookie, command = {
    cookie <- read.table(file.path(
      path_to_store(),
      "..",
      "data",
      "cookie.txt"
  ...
  tar_target(name = deliverables, command = {
    ...
    debug <- 0
    links <- c(
      "https://dmapps/api/ppt/project-years/",
      "https://dmapps/api/ppt/activities-full/"
    )
    ...
    year <- unlist(lapply(api_data, function(x) {
      x$project_year_obj$display_name
    }))
    ...
    df <- data.frame(
      "project_year_id" = project_year_id,
    ...
    p_ids <- lapply(
      API_DATA[[which(
        names(API_DATA) == "https://dmapps/api/ppt/project-years/"
      )]],
      function(x) x$project$id
    )
    ...
    for (i in seq_along(names(my_list))) {
      ...
      my_list[[i]] <- unlist(lapply(
        API_DATA[[which(
          names(API_DATA) == "https://dmapps/api/ppt/project-years/"
        )]][[which(p_ids == names(my_list)[i])[1]]]$project$year,
        function(x) x$id
      ))
    }
    ...
    assign_id <- function(df) {
      ...
      df$project_id <- sapply(df$project_year_id, function(id) {
        ...
        if (length(match_idx) > 0) {
          as.numeric(names(my_list)[match_idx[1]])
        } else {
          NA
        }
      })
    ...
    DD <- DD[
      which(!grepl("Agreement", DD$classification_display, ignore.case = TRUE)),
    ]
  ...
  tar_target(name = data_csas, command = {
    ...
    df <- data.frame('title' = title, 'date' = display_dates)
  }),

  tar_target(name = render_reports, command = {
    MPAs
    pillar_ecol_df
    Context
    flowerPalette
    ...
    for (i in seq_along(mpas)) {
      ...
      output_file <- paste0(
        output_dir,
        "/",
        make.names(paste0(names = mpas[i], ".html"))
      )
      render(
        input = rmd_file,
    ...
    output_file <- file.path(paste0(
      output_dir,
      "/",
      make.names(paste0(names = params$mpas, ".html"))
    ))
    render(
      input = rmd_file,
      output_file = output_file,
  ...
  tar_target(name = areas, command = {
    MPAs$NAME_E
  }),

  tar_target(name = objectives, command = {
    lapply(areas, function(x) data_objectives(type = "site", area = x))
  }),

  tar_target(name = Objectives_processed, command = {
    ...
    OBJECTIVES <- OBJECTIVES[which(unname(unlist(lapply(
      OBJECTIVES,
  ...
  tar_target(name = N_Objectives, command = {
  ...
  tar_target(name = Context, command = {
  ...
  tar_target(name = flowerPalette, command = {
    grades <- c("A", "B", "C", "D", "F")
  ...
  tar_target(name = indicatorFlower, command = {
  ...
  tar_target(name = odf, command = {
    O <- data.frame(
      objectives = c(
        0,
        unlist(Objectives_processed, use.names = FALSE),
        N_Objectives
      )
    )
    O$flower_plot <- 0
    ...
    get_first_four_words <- function(texts) {
      ...
      lapply(texts, function(text) {
        ...
        first_four_words <- paste(
          words[1:min(7, length(words))],
          collapse = " "
        ) # Concatenate the first four words (or fewer if there are not enough words)
    ...
    for (i in seq_along(O$objectives)) {
      ...
      if (!(O$objectives[i] == "0")) {
        keep <- which(
          tolower(get_first_four_words(fp$label_Objective)) ==
            tolower(get_first_four_words(ob)[[1]])
        )
        if (length(keep) > 1) {
          browser()
        }
...
"
  ...
  tar_target(name = data_WORMS_species_distributions, command = {
    distributions <- map(unique(data_obis$aphiaid), function(aphiaID) {
      url <- sprintf(
        "https://www.marinespecies.org/rest/AphiaDistributionsByAphiaID/%d",
        aphiaID
      )
      tryCatch(
        {
  ...
  tar_target(name = data_WORMS_species_distributions_polygons, {
    polygons <- data_WORMS_species_distributions |>
      ...
      unique() |>
      mutate(
        mrID = as.numeric(basename(locationID)),
  ...
  tar_target(data_musquash_coliform, command = {
    ...
    for (i in seq_along(sheets)) {
      ...
      times <- as.POSIXct(
        sub(".* - ", "", times[-(which(is.na(times) | times == "Station"))]),
        format = "%Y/%m/%d"
      )
  ...
  tar_target(name = data_musquash_nekton_occurence, command = {
    ...
    tmp2 <- tempfile()
    download.file(
      "https://catalogue.ogsl.ca/data/ecw/ca-cioos_4c93ac96-0a9f-41d5-9505-80a3b24c30ae/ecw_nekton-project_occurrence_2019-2021.csv",
      tmp,
      mode = "wb"
    )
    download.file(
      "https://catalogue.ogsl.ca/data/ecw/ca-cioos_4c93ac96-0a9f-41d5-9505-80a3b24c30ae/ecw_nekton-project_event_2019-2021.csv",
      tmp2,
      mode = "wb"
    )
  ...
  tar_target(control_polygons, command = {
    MPAs
    cp <- MPAs |>
  ...
  tar_target(name = creature_feature, command = {
    sptable <- data_inaturalist |>
      ...
      filter(
        ...
        phylum != "Ascomycota", # remove mushrooms and lichens
    ...
    for (i in 1:nrow(sptable)) {
      ...
      sptable$imageurl[i] <- {
        url <- try(
          read_html(sptable$exampleurl[i]) |>
      ...
      sptable$image_column[i] <- paste0(
        '<a href="',
        sptable$exampleurl[i],
        '" target="_blank">',
        '<img src="',
        sptable$imageurl[i],
        '" style="max-height: 100px; max-width: 100px; object-fit: cover;"/>',
        '<br>',
        sptable$rightsHolder[i],
        '</br>',
        '</a>'
      )

      if (is.na(sptable$commonname[i])) {
        spnames <- try(name_usage(
          sptable$speciesKey[i],
        ...
        if (!inherits(spnames, "try-error")) {
          if (nrow(spnames) > 0) {
            ...
            if (!is.null(cn)) {
              sptable$commonname[
                sptable$speciesKey[i] == sptable$speciesKey
              ] <- cn
            }
          }
        }
  ...
  tar_target(
    data_inseadistance_matrix,
    calc_in_sea_distance(
      cellsize = 5000,
      bioregion = st_union(regions),
      areas = MPAs[MPAs$NAME_E != "Non_Conservation_Area", ]
    )
  ),

  tar_target(
    data_protconn_EL_by_region,
    command = {
      mpas <- MPAs |>
      ...
      edgelist <- map(
        c(regions$NAME_E, mpas$NAME_E[mpas$region == regions$NAME_E]),
        function(x) {
          ind_ProtConn(
            distkm = data_inseadistance_matrix[
              rownames(data_inseadistance_matrix) != x,
              colnames(data_inseadistance_matrix) != x
            ],
            dkm = 50,
            bioregion = regions,
            ...
            mutate(
              region = regions$NAME_E,
              areaID = if_else(regions$NAME_E == x, NA, x)
            )
        }
      ) |>
        bind_rows()
      ...
      PCregion <- as.numeric(
        100 * sqrt(sum(edgelist$product[is.na(edgelist$areaID)])) / A
      )
      ...
      summaryEL <- edgelist |>
        ...
        group_by(areaID) |>
        mutate(
          PC = as.numeric(100 * sqrt(sum(product)) / A),
  ...
  tar_target(ind_otn_proportion_tags_detected_in_multiple_mpas, command = {
    ...
    proportion_in_different_mpa <- data.frame(
      areaID = unique(unlist(otn_areas)),
      number_in_mpa = NA,
      proportion_in_mpa_and_another = NA,
      connected_MPA = NA
    )

    for (i in seq_along(proportion_in_different_mpa$areaID)) {
      message(i)
      keep <- unname(which(sapply(otn_areas, function(x) {
        proportion_in_different_mpa$areaID[i] %in% x
      })))
      proportion_in_different_mpa$number_in_mpa[i] <- length(keep)

      if (!any(unname(unlist(lapply(otn_areas, length)))[keep] > 1)) {
        # Checking if in more than 1 MPA
        proportion_in_different_mpa$proportion_in_mpa_and_another[i] <- 0
      } else {
        multiple_mpas <- length(which(
          unname(unlist(lapply(otn_areas, length)))[keep] > 1
        ))
        proportion_in_different_mpa$proportion_in_mpa_and_another[i] <- round(
          multiple_mpas / length(keep) * 100,
          2
        )
        proportion_in_different_mpa$connected_MPA[i] <- paste0(
          unique(unlist(unname(otn_areas[keep])))[
            -which(
              unique(unlist(unname(otn_areas[keep]))) ==
                proportion_in_different_mpa$areaID[i]
            )
          ],
          collapse = ","
        )
    ...
    for (i in seq_along(unique(x$areaID))) {
      if (any(proportion_in_different_mpa$areaID == unique(x$areaID)[i])) {
        x$score[which(
          x$areaID == unique(x$areaID)[i]
        )] <- proportion_in_different_mpa$proportion_in_mpa_and_another[which(
          proportion_in_different_mpa$areaID == unique(x$areaID)[i]
        )]
        keep <- which(proportion_in_different_mpa$areaID == unique(x$areaID)[i])
        connected_mpa <- ifelse(
          proportion_in_different_mpa$proportion_in_mpa_and_another[keep] == 0,
          ".",
          paste0("(", proportion_in_different_mpa$connected_MPA[keep], ").")
        )

        ss <- paste0(
          "This protected area has had ",
          proportion_in_different_mpa$number_in_mpa[keep],
          " OTN tags detected in the area. Of those, ",
          proportion_in_different_mpa$proportion_in_mpa_and_another[keep],
          " % were detected in another MPA ",
          connected_mpa
        )
    ...
    for (i in seq_along(unique(x$areaID))) {
      ...
      k2 <- which(unname(sapply(otn_areas_old, function(xx) {
        name_of_interest %in% xx
      })))
      keep <- intersect(k1, k2)
      ...
      if (length(keep) > 0) {
        ...
        map <- ggplot() +
          ...
          labs(color = "Tag", x = "Longitude", y = "Latitude")

        # Store results
        x$plot[[i]] <- map
        x$data[[i]] <- tester[
          tester$areaID == name_of_interest,
          c("tag_id", "geometry")
        ]
  ...
  tar_target(ind_fish_length, command = {
    data <- data_rv_det |>
      mutate(
        longitude = LONGITUDE,
        latitude = LATITUDE,
  ...
  tar_target(ind_fish_weight, command = {
    data <- data_rv_det |>
      mutate(
        longitude = LONGITUDE,
        latitude = LATITUDE,
  ...
  tar_target(ind_haddock_counts, command = {
    data <- data_rv |>
  ...
  tar_target(ind_haddock_biomass, command = {
    data <- data_rv |>
      filter(COMM %in% c("HADDOCK")) |>
      mutate(
        longitude = LONGITUDE,
  ...
  tar_target(name = ind_QC_gulf_biogenic_habitat_representation, command = {
    process_indicator(
      data = data_QC_gulf_biogenic_habitat,
  ...
  tar_target(name = ind_species_representation, command = {
    data <- data_obis |>
      group_by(scientificName) |>
      reframe(
        subclass = unique(subclass[!is.na(subclass)])[1],
  ...
  tar_target(name = ind_musquash_infaunal_diversity, command = {
  ...
  tar_target(name = ind_musquash_nekton_diversity, command = {
  ...
  tar_target(name = ind_musquash_phosphate, command = {
    data <- data_musquash_eutrophication |>
  ...
  tar_target(name = ind_musquash_secchi, command = {
    data <- data_musquash_eutrophication |>
  ...
  tar_target(name = ind_musquash_coliform, command = {
  ...
  tar_target(ind_nitrate_inside_outside, command = {
    control_polygons
    MPAs
    data <- data_azmp_Discrete_Occupations_Sections |>
      dplyr::select(longitude, latitude, year, depth, nitrate)
  ...
  tar_target(ind_musquash_phosphate_inside_outside, command = {
    control_polygons
    MPAs
  ...
  tar_target(ind_musquash_coliform_inside_outside, command = {
    control_polygons
    MPAs
  ...
  tar_target(ind_protconn, command = {
  ...
  tar_target(name = ind_sst, command = {
    ind_placeholder(
      ind_name = "Sea Surface Temperature",
      areas = MPAs,
  ...
  tar_target(name = ind_temp_at_depth, command = {
    ind_placeholder(
      ind_name = "Temperature at Depth",
      areas = MPAs,
      readiness = "Readily Available",
      source = "AZMP",
      objectives = c(
        "Protect continential shelf habitats and associated benthic and demersal communities"
      )
    )
  }), # Environmental Representativity, Ocean Conditions

  tar_target(name = ind_sea_surface_salinity, command = {
    ind_placeholder(
      ind_name = "Sea Surface Salinity",
      areas = MPAs,
      readiness = "Readily Available",
      source = "AZMP",
      objectives = c(
  ...
  tar_target(name = ind_subsurface_salinity, command = {
    ind_placeholder(
      ind_name = "Subsurface Salinity",
      areas = MPAs,
      readiness = "Readily Available",
      source = "AZMP",
      objectives = c(
  ...
  tar_target(name = ind_nitrate, command = {
    ind_placeholder(
      ind_name = "Nitrate",
      areas = MPAs,
      readiness = "Readily Available",
  ...
  tar_target(name = ind_ave_position_of_shelf_slope_front, command = {
    ind_placeholder(
      ind_name = "Average Position of Shelf Slope Front",
      areas = MPAs,
      readiness = NA,
      source = "BNAM",
      objectives = c(
  ...
  tar_target(name = ind_phytoplankton_biomass_and_diversity, command = {
    ind_placeholder(
      ind_name = "Phytoplankton Biomass and Diversity",
      areas = MPAs,
      readiness = NA,
      source = "AZMP",
      objectives = c(
  ...
  tar_target(name = ind_fish_eggs_and_larve, command = {
    ind_placeholder(
      ind_name = "Fish Eggs and Larve",
      areas = MPAs,
      readiness = NA,
      source = "Dedicated Surveys",
      objectives = c(
        "Support productivity objectives for groundfish species of Aboriginal, commercial, and/or recreational importance, particularly NAFO Division 4VW haddock",
        "Conserve and protect biological productivity across all trophic levels so that they are able to fulfill their ecological role in the ecosystems of the MPA"
      )
  ...
  tar_target(name = ind_seasonal_presence_cetaceans, command = {
    ind_placeholder(
      ind_name = "Seasonal Presence/Absence of Cetaceans",
      areas = MPAs,
      readiness = NA,
      source = "Passive Acoustics",
  ...
  tar_target(name = ind_seasonal_presence_pelagics, command = {
    ind_placeholder(
      ind_name = "Seasonal Presence/Absence of Large Pelagics",
      areas = MPAs,
      readiness = NA,
      source = "Tagging",
      objectives = c("Protect unique, rare, or sensitive ecological features")
  ...
  tar_target(name = ind_seasonal_presence_seabirds, command = {
    ind_placeholder(
      ind_name = "Seasonal Presence/Absence of Seabirds",
      areas = MPAs,
      readiness = NA,
      source = "CWS Data",
      objectives = c("Protect unique, rare, or sensitive ecological features")
  ...
  tar_target(name = ind_proportion_demersal_fish, command = {
    ind_placeholder(
      ind_name = "Proportion of Large and Small Demersal Fish Species",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_abundance_invasive_species_webmr, command = {
    ind_placeholder(
      ind_name = "Abundance of Aquatic Invasive Species in WEBMER",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_biomass_groundfish_prey_webmr, command = {
    ind_placeholder(
      ind_name = "Biomass of Groundfish Prey Species within WEBMR",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_abundance_sea_pens_webmr, command = {
    ind_placeholder(
      ind_name = "Abundance of Sea Pens within WEBMR",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_community_comp_epibenthic_infaunal, command = {
    ind_placeholder(
      ind_name = "Community Composition of Epibenthic and Infaunal Benthic Communities",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_biomass_bioturbators, command = {
    ind_placeholder(
      ind_name = "Biomass of Bioturbators",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_spatial_extent_ebsa_webmr, command = {
    ind_placeholder(
      ind_name = "Spatial Extent of EBSA with WEBMR",
      areas = MPAs,
      readiness = NA,
      source = "Dedicated Surveys",
      objectives = c(
  ...
  tar_target(name = ind_size_distribution_groundfish, command = {
    ind_placeholder(
      ind_name = "Size Distribution of Select Groundfish Species",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_condition_groundfish, command = {
    ind_placeholder(
      ind_name = "Condition of Select Groundfish Species",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_fecundity_groundfish, command = {
    ind_placeholder(
      ind_name = "Fecundity of Select Groundfish Species",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_community_comp_demersal, command = {
    ind_placeholder(
      ind_name = "Community Composition of Demersal Fish",
      areas = MPAs,
      readiness = "Readily Available",
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_distribution_juv_haddock_habitat, command = {
    ind_placeholder(
      ind_name = "Distriburion of Juvenile Haddock Habitat",
      areas = MPAs,
      readiness = NA,
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_distribution_key_fish_habitat, command = {
    ind_placeholder(
      ind_name = "Distribution of Key Fish Species Habitat",
      areas = MPAs,
      readiness = "Readily Available",
      source = "RV Survey",
      objectives = c(
  ...
  tar_target(name = ind_unauthorized_fishing, command = {
    ind_placeholder(
      ind_name = "Amount of Unauthorized Fishing",
      areas = MPAs,
      readiness = NA,
      source = "C&P",
      objectives = c(
  ...
  tar_target(name = ind_seabed_disruption_in_around_webmr_fishing, command = {
    ind_placeholder(
      ind_name = "Seabed area in and surrounding WEBMR swept by bottom-tending commercial fishing gear",
      areas = MPAs,
      readiness = NA,
      source = "AIS Data",
      objectives = c(
  ...
  tar_target(name = ind_total_annual_landings, command = {
    ind_placeholder(
      ind_name = "Total annual landings form 4W for directed groundfish fisheries and common bycatch only stocks",
      areas = MPAs,
      readiness = NA,
      source = "Commercial Catch Information",
      objectives = c(
        "Manage human activities to minimize impacts on other commercial and non-commercial living resources",
        "Help maintain healthy populations of species of Aboriginal, commercial, and/or recreational importance"
      )
  ...
  tar_target(name = ind_seabed_disruption_in_around_webmr_research, command = {
    ind_placeholder(
      ind_name = "Seabed area in and surrounding WEBMR swept by bottom-tending commercial fishing gear for research and monitoring",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_offshore_wind_developments, command = {
    ind_placeholder(
      ind_name = "Number of offshore wind developments within in the vacinity of WEBMR",
      areas = MPAs,
      readiness = NA,
      source = "Offshore Energy Regulator",
      objectives = c(
  ...
  tar_target(name = ind_contaminant_concentration, command = {
    ind_placeholder(
      ind_name = "Concentrations of Contaminants by Type",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_anthropogenic_debris, command = {
    ind_placeholder(
      ind_name = "Quantity of Anthropogenic Debris",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_biomass_abund_distribution_musquash, command = {
    ind_placeholder(
      ind_name = "Total biomass, abundance, and dustribution of key species in each trophic level",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_species_at_risk, command = {
    ind_placeholder(
      ind_name = "Number of at risk species within MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_disturbed_area, command = {
    ind_placeholder(
      ind_name = "Total area and location of habitat type and proportion and frequency disturbed or lost",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_sediment_regime, command = {
    ind_placeholder(
      ind_name = "Hydrodynamic and sediment regime in estuary",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
...
"
  ...
  tar_target(name = ind_cpue, command = {
    ind_placeholder(
      ind_name = "Commercial and recreational fishing catch per unit effort",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_bycatch, command = {
    ind_placeholder(
      ind_name = "By-catch number per impacted species",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_abundance_nbw, command = {
    ind_placeholder(
      ind_name = "Abundance of Northern Bottlenose Whales",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_mpa_use_nbw, command = {
    ind_placeholder(
      ind_name = "Use of the Gully by Northern Bottlenose Whales ",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = c(
  ...
  tar_target(name = ind_population_characteristics, command = {
    ind_placeholder(
      ind_name = "Size, age, and sex structure of the Scotian Shelf population of Northern Bottlenose Whales",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_fresh_scars, command = {
    ind_placeholder(
      ind_name = "% of individuals in the Scotian Shelf Nortyhern Bottlenose population showing fresh scars ",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_genetic_diversity_nbw, command = {
    ind_placeholder(
      ind_name = "Genetic diversity within the Northern Bottlenose Whales",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_blubber_contaminants, command = {
    ind_placeholder(
      ind_name = "Level of contaminants in the blubber of individuals in the Scotian Shelf population of Northern Bottlenose Whales ",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_rel_abundance_cetaceans, command = {
    ind_placeholder(
      ind_name = "Relative abundances of cetaceans beside Northern Bottlenose Whales in The Gully",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_cetacean_presence, command = {
    ind_placeholder(
      ind_name = "Cetacean presence and activity in the MPA, year-round",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_strandings, command = {
    ind_placeholder(
      ind_name = "Number of reported strandings of Scotian Shelf Northern Bottlenose Whales",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_ship_strikes, command = {
    ind_placeholder(
      ind_name = "Number of reported ship strikes on cetaceans in or near the Gully and of strikes on Scotian Shelf Northern Bottlenose Whales elsewhere",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_entanglement_gully, command = {
    ind_placeholder(
      ind_name = "Number of reported gear entanglements of cetaceans in or near the Gully and of entanglements of Scotian Shelf Northern Bottlenose Whales elsewhere",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_human_interaction, command = {
    ind_placeholder(
      ind_name = "Number of reports of other interactions between human activities and cetaceans in or near the Gully and of interactions with Scotian Shelf Northern Bottlenose Whales",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_coral_distribution, command = {
    ind_placeholder(
      ind_name = "Coral distribution, density, and size structure by species at select monitoring sites",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_coral_diversity, command = {
    ind_placeholder(
      ind_name = "Coral Diversity at selcted monitoring sites",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_coral_proportions, command = {
    ind_placeholder(
      ind_name = "Proportions of live and dead corals, by species, at selected monitoring sites within the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_zoanthid_overgrowth, command = {
    ind_placeholder(
      ind_name = "Proportion on live corals at selected monitoring sites with the MPA that show zoanthid over-growths, and extent of over-growth in affected colonies",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_trawl_vunerable, command = {
    ind_placeholder(
      ind_name = "Relative abundances, size distributions and diversity of selected groundfish and trawl-vunerable invertebrates in Zone 3 of the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_longline_vunerable, command = {
    ind_placeholder(
      ind_name = "Relative abundances, size distributions and diversity of selected longline-vunerable species in Zones 2 and 3 of the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_trap_vunerable, command = {
    ind_placeholder(
      ind_name = "Relative abundances, size distributions and diversity of selected trap-vunerable species in Zonea 1 and 2 of the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_mesopelagic_nektonic, command = {
    ind_placeholder(
      ind_name = "Relative abundances, size distributions and diversity of selected mesopelagic nektonic species in Zones 1 and 2 of the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_environmental_conditions_azmp_lines, command = {
    ind_placeholder(
      ind_name = "Temperature, salinity, oxygen concentration, light levels, chlorophyll, pigments and nutrients in waters flowing into and past the MPA, as measured on the Louisbourg Line, the Halifax Line and the Extended Halifax Line",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_physical_biological_surface_properties, command = {
    ind_placeholder(
      ind_name = "Physical (temperature, salinity, wind, sea-surface height) and biological (ocean colour) sea surface properties in the MPA and the surrounding region",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_weather_station_buoy_sites, command = {
    ind_placeholder(
      ind_name = "Weather conditions at the Sable Island weather station, Banquereau and Laurentian Fan weather-buoy sites, including wind direction and speed, air pressure and sea-level air temperatures, and at buoy sites sea surface temperatures, wave height and dominant wave period",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_water_masses, command = {
    ind_placeholder(
      ind_name = "Three-dimensional distribution and movements of water masses within and around the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_acoustic_scattering, command = {
    ind_placeholder(
      ind_name = "Acoustic scattering in the water column within the MPA (as a measure of mesopelagic and zooplankton densities and distribution)",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_seabird_abundance, command = {
    ind_placeholder(
      ind_name = "Distribution and abundance of seabird species within the MPA, including an index of planktivorous seabird species",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_vessel_transits, command = {
    ind_placeholder(
      ind_name = "Number of transits of the MPA by vessels other than pleasure craft, broken down into mercantile vessels, surface naval vessels and fishing vessels not fishing in the area",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_vessel_operation, command = {
    ind_placeholder(
      ind_name = "Hours of operation within the MPA by vessels other than commercial fishing vessels or pleasure craft, broken down into research and monitoring vessels, other government vessels, and ecotourism vessels",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_fishing_effort, command = {
    ind_placeholder(
      ind_name = "Commercial fishing effort within the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_fishing_effort_nearby, command = {
    ind_placeholder(
      ind_name = "Commercial fishing effort in close proximity to the MPA boundary",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_corals_removed, command = {
    ind_placeholder(
      ind_name = "Quantities of corals removed from within the MPA by commercial fishing or research activity",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_target_org_removed_gully, command = {
    ind_placeholder(
      ind_name = "Quantities of taget organisims removed from within the MPA and of bycatch organisms other than corals removed from the MPA by commercial fishing",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_organisms_removed, command = {
    ind_placeholder(
      ind_name = "Quantities of organisms besides corals removed from the MPA by research activities",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_seabed_swept, command = {
    ind_placeholder(
      ind_name = "Area of seabed swept by bottom-tending mobile research and monitoring gear in the MPA as a total and subdivided by seabed habitat type",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_seabed_occupied, command = {
    ind_placeholder(
      ind_name = "Length of lines of, and seabed area occupied by, bottom-set fixed commercial fishing and research and monitoring gears within the MPA, as totals and subdivided by seabed habitat type",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_petroleum_activity, command = {
    ind_placeholder(
      ind_name = "Number and types of offshore-petroleum exploration and development activities on the eastern Scotian Shelf",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_ballast, command = {
    ind_placeholder(
      ind_name = "Number of ships’ ballast-water exchanges in the proximity of the MPA and the quantities of ballast exchanged",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_other_discharge, command = {
    ind_placeholder(
      ind_name = "Number, quantities and type of other discharges from shipping within or in proximity to the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_floating_debris_gully, command = {
    ind_placeholder(
      ind_name = "Quantity of floating debris (i.e., large objects) in the Gully MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_seabed_debris_gully, command = {
    ind_placeholder(
      ind_name = "Quantity of anthropogenic debris on the seabed at selected monitoring sites in the Gully MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_anthropogenic_sound, command = {
    ind_placeholder(
      ind_name = "Quantitative characteristics of anthropogenic sound within the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_physandbio_properties, command = {
    ind_placeholder(
      ind_name = "Temperature, salinity, oxygen concentration, light levels, chlorophyll, pigments, nutrients and zooplankton within the AOI and both upstream and downstream",
      areas = MPAs,
      readiness = NA,
      source = AZMP,
      objectives = NA
  ...
  tar_target(name = ind_physandbio_seasurface_properties, command = {
    ind_placeholder(
      ind_name = "Physical (e.g. temperature, salinity, wind, sea-surface height) and biological (e.g. ocean colour) sea surface properties in the MPA and the surrounding region",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_weather_condition, command = {
    ind_placeholder(
      ind_name = "Weather conditions at the Sydney Airport and Fourchu Head weather stations, including wind direction and speed, air pressure and sea-level air temperature",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_ice_cover, command = {
    ind_placeholder(
      ind_name = "Extent of ice cover within and around the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_fluxes, command = {
    ind_placeholder(
      ind_name = "Fluxes, other than those of nekton, across the boundaries of the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_exchanges, command = {
    ind_placeholder(
      ind_name = "Bentho-pelagic exchanges",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_plankton_production, command = {
    ind_placeholder(
      ind_name = "Phytoplankton production and the timing and intensity of the spring bloom in the MPA and the surrounding region",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_mesozooplankton_community, command = {
    ind_placeholder(
      ind_name = "Mesozooplankton community composition within the AOI and both upstream and downstream",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_harmful_algae, command = {
    ind_placeholder(
      ind_name = "Blooms of harmful algal in or near the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_seabed_feature_extent, command = {
    ind_placeholder(
      ind_name = "Spatial extent of identified distinctive seabed features of the AOI",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_resource_species_abundance, command = {
    ind_placeholder(
      ind_name = "Population-wide abundances and size distributions of those populations of resource species which utilize the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_groundfish_abundance_sab, command = {
    ind_placeholder(
      ind_name = "Relative abundances, biomasses, size distributions and population fecundities of selected groundfish and invertebrates, plus diversity and community composition of trawl-vulnerable species, in appropriate portions of the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_nekton_abundance, command = {
    ind_placeholder(
      ind_name = "Relative abundances, biomasses and size distributions of selected mesopelagic nekton and micronekton species in the Laurentian Channel portion of the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_compared_groundfish_abundance, command = {
    ind_placeholder(
      ind_name = "Relative abundances, biomasses, size distributions and population fecundities of selected groundfish and invertebrates, plus diversity and community composition of trawl-vulnerable species, in comparable areas outside the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_compared_longline_vunerable, command = {
    ind_placeholder(
      ind_name = "Relative abundances, biomasses, size distributions and population fecundities of selected longline-vulnerable species in comparable areas outside the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_fish_nekton_fluxes, command = {
    ind_placeholder(
      ind_name = "Fluxes of fish and other nekton across the boundaries of the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_mammal_bird_reptile, command = {
    ind_placeholder(
      ind_name = "Distributions, relative abundances, diversity, community composition and activities of mammals, birds and reptiles in the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_seal_breeding, command = {
    ind_placeholder(
      ind_name = "Grey and harp seal breeding in the vicinity of the AOI",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_seabird_nesting, command = {
    ind_placeholder(
      ind_name = "Seabird nesting in the vicinity of the AOI",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_trophic_relationships, command = {
    ind_placeholder(
      ind_name = "Trophic relationships in the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_ecosystem_function, command = {
    ind_placeholder(
      ind_name = "Ecosystem function in the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_marxan_input, command = {
    ind_placeholder(
      ind_name = "Data inputs to the MARXAN analysis",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_bottomset_length, command = {
    ind_placeholder(
      ind_name = "Length of bottom-set fixed commercial fishing, research and monitoring lines set within the MPA, both as totals and subdivided by zone or seabed habitat type",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_vertical_lines, command = {
    ind_placeholder(
      ind_name = "Number of vertical lines and length of midwater lines set within the MPA as part of commercial fishing, research or monitoring gears, both as a total and subdivided by zone",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_bait, command = {
    ind_placeholder(
      ind_name = "Quantities and types of baits introduced to the MPA as part of commercial fishing, research or monitoring gears, both as a total and subdivided by zone",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_development_sab, command = {
    ind_placeholder(
      ind_name = "Number and types of seabed cables, offshore-petroleum exploration and development activities, other mineral exploration and development activities, channel dredging projects or other large-scale engineering works in the general vicinity of the MPA, including any within the MPA itself",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_anchoring, command = {
    ind_placeholder(
      ind_name = "Incidents of vessels anchoring within the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_other_discharge_offshore, command = {
    ind_placeholder(
      ind_name = "Number, quantities and type of discharges from vessels of all kinds or from offshore installations within, or in proximity to the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_other_discharge_coastal, command = {
    ind_placeholder(
      ind_name = "Number, quantities and type of discharges from coastal sources within or in proximity to the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(name = ind_ocean_noise_sab, command = {
    ind_placeholder(
      ind_name = "Characterization of deep-water natural and anthropogenic noise within the MPA",
      areas = MPAs,
      readiness = NA,
      source = NA,
      objectives = NA
  ...
  tar_target(objective_indicators, command = {
    ...
    indicator_objectives <- indicator_objectives[
    ...
    for (i in seq_along(ped_objectives)) {
      ...
      if (
        x$Framework[which(x$Objective == indicator_objectives[i])] %in%
          MPAs$NAME_E
      ) {
        keep2 <- which(
          ped$areaID ==
            x$Framework[which(x$Objective == indicator_objectives[i])]
        )
    ...
    new_objectives <- x$Objective[which(
      !(x$Objective %in% names(ped_objectives))
    )]
  ...
  tar_target(theme_ocean_conditions, command = {
    x <- rbind(
    ...
    for (i in seq_along(x$areaID)) {
      ...
      X <- x[i, ]
      keep <- which(
        pillar_ecol_df$areaID == X$areaID &
          pillar_ecol_df$indicator == X$indicator
      )
  ...
  tar_target(theme_ocean_structure_and_movement, command = {
    x <- rbind(
      ind_otn_proportion_tags_detected_in_multiple_mpas[, setdiff(
        names(ind_otn_proportion_tags_detected_in_multiple_mpas),
        c("data", "plot")
      )],
      ind_surface_height[, setdiff(
        names(ind_surface_height),
        c("data", "plot")
      )],
      ind_stratification[, setdiff(
        names(ind_stratification),
        c("data", "plot")
      )]
    )
  ...
  tar_target(theme_primary_production, command = {
    x <- rbind(
      ...
      ind_bloom_amplitude[, setdiff(
        names(ind_bloom_amplitude),
        c("data", "plot")
      )],
  ...
  tar_target(theme_secondary_production, command = {
    x <- rbind(
      ind_zooplankton[, setdiff(names(ind_zooplankton), c("data", "plot"))],
      ind_zooplankton_community_composition[, setdiff(
        names(ind_zooplankton_community_composition),
        c("data", "plot")
      )]
    )
  ...
  tar_target(theme_marine_mammals_and_other_top_predators, command = {
    x <- rbind(
      ind_musquash_birds_sample_coverage[, setdiff(
        names(ind_musquash_birds_sample_coverage),
        c("data", "plot")
      )]
    )
  ...
  tar_target(theme_trophic_structure_and_function, command = {
    data.frame(
      areaID = NA,
      indicator = NA,
      type = NA,
  ...
  tar_target(theme_benthic_environment, command = {
    data.frame(
      areaID = NA,
      indicator = NA,
      type = NA,
  ...
  tar_target(theme_fish_and_fishery_resources, command = {
    x <- rbind(
      ind_fish_length[, setdiff(names(ind_fish_length), c("data", "plot"))],
      ind_fish_weight[, setdiff(names(ind_fish_weight), c("data", "plot"))],
      ind_haddock_counts[, setdiff(
        names(ind_haddock_counts),
        c("data", "plot")
      )],
      ind_haddock_biomass[, setdiff(
        names(ind_haddock_biomass),
        c("data", "plot")
      )]
    )
  ...
  tar_target(theme_anthropogenic_pressure_and_impacts, command = {
    x <- data.frame(
      areaID = NA,
      indicator = NA,
      type = NA,
  ...
  tar_target(theme_table, command = {
    rbind(
      theme_ocean_conditions,
    ...
    x <- pedf_filtered |>
      ...
      mutate(
        tab = paste0(
          "tab_",
          seq(
            length(APPTABS$flower) + 1,
            length(APPTABS$flower) + length(objective)
          )
        )
      )
  ...
  tar_target(name = project_widget_choices, command = {
    distinct_rows <- unique(all_project_geoms[c(
      "project_short_title",
      "PPTID",
      "source"
    )])
    ...
    unique(paste0(
      distinct_rows$project_short_title,
      " (",
      ifelse(
        is.na(distinct_rows$PPTID),
        distinct_rows$source,
        distinct_rows$PPTID
      ),
      ")"
    ))
  ...
  tar_target(name = MPA_report_card, command = {
    mrc <- left_join(
      MPAs,
      pillar_ecol_df |>
        filter(indicator %in% MPAs$NAME_E, areaID != "Non_Conservation_Area") |>
        calc_group_score(grouping_var = "indicator") |>
        mutate(grade = if_else(is.nan(score), NA, calc_letter_grade(score))),
  ...
  tar_target(
    ...
    command = {
      ...
      all_project_geoms_single_obs_per_row <- all_project_geoms |>
        filter(!is.na(PPTID), areaID != "Non_Conservation_Area") |>
      ...
      all_project_geoms_single_obs_per_row |>
        ...
        tidyr::complete(
          PPTID,
          areaID = MPAs$NAME_E[MPAs$NAME_E != "Non_Conservation_Area"],
          fill = list(sites = 0)
        ) |>
        left_join(project_samples_total, by = "PPTID") |>
        mutate(percent_sites_in_mpa = sites / totalsites) |>
        rename(area = areaID, project_id = PPTID) |>
  ...
  tar_target(
    ...
    command = {
      ...
      system(paste0(
        ...
        ' "',
        serveruser,
        '@mar-spa.ent.dfo-mpo.ca:/home/rdaigle/MarConsNetTargets/app_targets" '
      ))
      ...
      system(paste0(
        ...
        ' "',
        serveruser,
        '@mar-spa.ent.dfo-mpo.ca:/home/rdaigle/MarConsNetTargets/app_targets/objects/" '
      ))
      ...
      datafiles <- list.files(
        file.path(dirname(path_to_store()), 'data'),
        include.dirs = FALSE
      )
      datadirs <- list.dirs(
        file.path(dirname(path_to_store()), 'data'),
