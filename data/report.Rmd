...
for (i in seq_along(keeper2)) {
  ...
  for (j in seq_along(keeper2[[i]]$Indicator)) {
    ...
    if (!(keeper2[[i]]$Plot[j] == "0")) {
      ...
      plot(st_geometry(mapk$area), col = "gray", main = "Spatial Visualization", 
     xlab = "Longitude", ylab = "Latitude", border = NA)

# Add the "outside" polygons
plot(st_geometry(mapk$outside), col = "red", add = TRUE, border = NA)

# Add circle markers for points
points(mapk$longitude, mapk$latitude, col = "black", pch = 20)
