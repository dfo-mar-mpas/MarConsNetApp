...
cat("Disclaimer: The cost information is based on the funding information in the project planning tool. The amount is based on percentage of samples taken in the area of interest.\n\n")
...
try(dataSPA::plotSPA(om=om_in_site, which="omBar"))

proj_titles <- NULL
for (i in seq_along(com$project_id)) {
  proj_titles[i] <- unique(pillar_ecol_df$project_short_title[which(pillar_ecol_df$PPTID == com$project_id[i])])
}

proj <- unique(com$project_id)


cat(paste0("The below investment information is based on the following internal DFO projects that have had samples in ", state$mpas," :", paste0(paste(proj, " (", proj_titles, ")", sep = ""), collapse="; ")))

} else {
  cat("There is no internal investment information available for this area.")
}
...
```{r, echo=FALSE}
try(dataSPA::plotSPA(om=om_in_site, which="omBar"), silent=TRUE)
