...
for (i in seq_along(n_objectives)) {
  ...
  if (!(length(keep) == 0)) {
    ...
    objective_flower$score[which(!(grepl(n_objectives[i], objective_flower$objectives)))] <- NA
...
if (!(length(S_Objectives) == 0)) {
  ...
  for(i in seq_len(nrow(dt_data))) {
    ...
    cat('<td>', dt_data$Objective[i], '</td>')
    cat('<td>', dt_data$Grade[i], '</td>')
    cat('</tr>\n')
  }
...
cat(sprintf('<details><summary><strong><u>OBJECTIVE: %s</u></strong></summary>\n\n', toupper(textO[i])))
  ...
  if (!(length(keep) == 0)) {
    ...
    objective_flower <- pillar_ecol_df[k2,]
    objective_flower$score[which(!(grepl(textO[i], objective_flower$objectives)))] <- NA
...
for (i in seq_along(keeper2)) {
  ...
  #  which(
   #   pillar_ecol_df$indicator == keeper2[[i]]$Indicator[j] &
    #  pillar_ecol_df$areaID == params$mpas
    #)
  #]
#))
