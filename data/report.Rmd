...
if (any(pillar_ecol_df$status_statement == "No features are represented.")) {
...
div(
  ...
  tags$p(investment),
  tags$p(paste0(state$mpas, " has an overall ecological status score of ", area_grade, ".")),
...
maps <- maps %>% 
  addPolygons(data=MPAs$geoms[which(MPAs$NAME_E == state$mpas)], fillOpacity = 0.05, weight=2, color=border_color)
...
for (i in seq_along(n_objectives)) {
  ...
  if (state$mpas %in% regions$NAME_E) {
    ...
    k2 <- which(pillar_ecol_df$areaID == state$mpas)
  }
  ...
  for (j in seq_along(keep)) {
    ...
    # FINDING PLOT
      keep1 <- which(grepl(make.names(state$mpas), image_files, ignore.case=TRUE)) # Which are from the correct area
