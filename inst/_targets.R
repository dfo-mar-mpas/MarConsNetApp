      ...
      if (!(length(quality_keep) == 0)) {
        ...
        if (length(unique(x$year[quality_keep])) == 1) {
          if (!(is.na(unique(x$year[quality_keep])))) {
            ...
            statements <- mapply(
              function(a) paste0(a, ": There are no quality statements available for GEOMETRYCOLLECTION type"),
              areas_with_indicators
            )
            network_quality_statement[i] <- paste0(paste0(unlist(unname(statements)), collapse=' ; '))

          } else if (any(grepl("POLYGON", class(x$geometry[quality_keep][1])))) {
            statements <- mapply(
              function(a) paste0(a, ": There are no quality statements available for POLYGON type"),
              areas_with_indicators
            )
