raw_data_targets <- list(
  ...
  tar_target(control_polygons, command = {
    ...
    cp <- MPAs |>
      ...
      rowwise() |>
      mutate(
        twenty_km = st_difference(st_buffer(geoms, 20000), geoms),
        forty_km = st_difference(
          st_buffer(geoms, 40000),
          st_buffer(geoms, 20000)
        ),
        sixty_km = st_difference(
          st_buffer(geoms, 60000),
          st_buffer(geoms, 40000)
        ),
        eighty_km = st_difference(
          st_buffer(geoms, 80000),
          st_buffer(geoms, 60000)
        )
      ) |>
      ungroup() |>
...
2017,2004,2017,2017,2017,2016,2019,2016,1998,2004,1996,2004,2004,1998,2002,2014,
  ...
  tar_target(
    ...
    command = {
      ...
      DD <- DD[
        which(
          !grepl("Agreement", DD$classification_display, ignore.case = TRUE)
        ),
      ]
      DD <- DD[, c(
        "project_id",
