...
p <- pillar_ecol_df[which(pillar_ecol_df$areaID %in% MPAs$NAME_E),]
...
for (i in seq_along(MPAs$NAME_E)) {
  mpa_of_interest <- MPAs$NAME_E[i]

if (mpa_of_interest %in% MPA_report_card$NAME_E) {
  fill_color <- ifelse(
    is.na(MPA_report_card$grade[which(MPA_report_card$NAME_E == mpa_of_interest)]),
    "#EDEDED",
    flowerPalette[[MPA_report_card$grade[which(MPA_report_card$NAME_E == mpa_of_interest)]]]
  )
  ...
  map <- map %>% 
    addPolygons(
      data = MPAs$geoms[which(MPAs$NAME_E == mpa_of_interest)],
...
keeper <- pillar_ecol_df[which(pillar_ecol_df$areaID %in% MPAs$NAME_E), ]
...
for (i in seq_along(BINS)) {
  ...
  # Scotian shelf only
  # Sort KK by indicator
  KK <- KK[order(KK$indicator), ]
