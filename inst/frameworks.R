framework_targets <- list(
  ...
  tar_target(bin_biodiversity_SpeciesDiversity_df, {
    bin <- aggregate_groups(
      ...
      # ind_species_richness_eDNA, #TODO: add back in when it works
  ...
  tar_target(bin_habitat_Uniqueness_df, {
    bin <- aggregate_groups(
      ...
      weights_ratio = 1,
      weights_sum = 1,
      # ind_QC_gulf_biogenic_habitat_representation, #TODO: add back in when it works
  ...
  tar_target(bin_productivity_BiomassMetrics_df, {
    bin <- aggregate_groups(
      ...
      ind_distinctive_benthic_characteristics_macroalgae,
