...
new_row <- ecol_obj_habitat_df[
  which(
    ecol_obj_habitat_df$indicator == "Bloom Amplitude" &
      grepl("Emerald Bank", ecol_obj_habitat_df$areaID)
  ),
]
...
d_pedf <- data_pillar_ecol_df[
  -which(data_pillar_ecol_df$areaID == "Non_Conservation_Area"),
]

MPA_report_card <- left_join(
  MPAs,
  d_pedf |>
...
tar_target(
  ...
  command = {
    ...
    for (i in seq_along(ppt)) {
      ...
      OM$number_of_project_stations[which(
        OM$project_id == as.numeric(names(ppt)[i])
      )] <- combinations # FIXME: Note, need to get the same to see if they happened at different times
    }
    ...
    for (i in seq_along(OM)) {
      price_per_station[[i]] <- sum(OM[[i]]$amount) /
        unique(OM[[i]]$number_of_project_stations)
    }
    ...
    for (i in seq_along(MPA$NAME_E)) {
      ...
      # TEST
      if (!(length(ppt) == 0)) {
        percent_sites_in_mpa[[i]] <- data.frame(
          project_id = names(price_per_station),
          percent_sites_in_mpa = rep(NA, length(names(price_per_station))),
          price_per_station = rep(NA, length(names(price_per_station))),
          area = MPA$NAME_E[i]
        )
      } else {
        percent_sites_in_mpa[[i]] <- data.frame(
          project_id = NA,
          percent_sites_in_mpa = NA,
          price_per_station = NA,
          area = MPA$NAME_E[i]
        )
        next
      }
      ...
      for (j in seq_along(names(price_per_station))) {
        ...
        if (!(is.null(result))) {
          ...
          total_sites <- unique(
            OM[[which(names(OM) == pps)]]$number_of_project_stations
          )

          percent_sites_in_mpa[[i]]$percent_sites_in_mpa[j] <- sites_in_mpa /
            total_sites *
            100
          percent_sites_in_mpa[[i]]$price_per_station[
            j
          ] <- price_per_station[[which(names(price_per_station) == pps)]]
        } else {
