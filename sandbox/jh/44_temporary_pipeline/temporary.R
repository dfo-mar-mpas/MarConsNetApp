...
indicator_objectives <- trimws(
  unique(unlist(strsplit(pillar_ecol_df$objectives, ";;;"))),
  'both'
)
...
for (i in seq_along(ped_objectives)) {
  ...
  keep <- which(grepl(
    ...
    pillar_ecol_df$objectives,
    fixed = TRUE
  ))
  if (
    x$Framework[which(x$Objective == indicator_objectives[i])] %in% MPAs$NAME_E
  ) {
    keep2 <- which(
      pillar_ecol_df$areaID ==
