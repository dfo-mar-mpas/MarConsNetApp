...
tar_load(c(MPAs,
           ...
           climate_change,
           salary,
           objective_tabs), store=path_to_store())
...
investment <- paste0(
  ...
  "% of O&M funding to the protected areas based on the data embedded in this project."
)
} else {
...
cat("Protected Area was established in ", MPAs$date_of_establishment[which(MPAs$NAME_E == params$mpas)], ".")
cat("")
...
cat("Samples in the last calendar year,", as.numeric(format(Sys.time(), "%Y"))-1, ", are as follows. Note this is based solely on data that have a time component associated with them.")
```
...
cat(sprintf(
  ...
  for (j in seq_along(keep)) {
    ...
    # END PLOT
      ...
      table_df <- data.frame("Rationale"=pillar_ecol_df2$indicator_rationale[keep[j]],
