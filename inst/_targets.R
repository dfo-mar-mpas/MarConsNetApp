...
list(
  ...
  tar_target(
    ...
    command = {
      ped <- pillar_ecol_df
      ot <- data.frame(
        objectives = objectives_df$Objective,
        tab = NA,
        area = objectives_df$Framework
      )
  ...
  tar_target(all_project_geoms, {
    all_indicator_project_geoms |>
      group_by(project_short_title, areaID, PPTID, source, geometry, year) |>
      summarise(type = paste(type, ";;"), scale = paste(scale, ";;")) |>
      st_make_valid() |>
      distinct()
  }),

  tar_target(labels, command = {
  ...
  tar_target(map_palette, command = {
    data.frame(
      "Project" = labels,
      "Color" = palette <- viridis::viridis(length(labels))
    )
  }),
