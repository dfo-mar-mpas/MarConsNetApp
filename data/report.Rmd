...
```{r}
try({inatoccs <- occ_data(geometry = st_as_text(st_as_sfc(st_bbox(MPAs[MPAs$NAME_E == state$mpas,]))),institutionCode = "iNaturalist")

sptable <-  inatoccs$data |> 
  filter(!is.na(speciesKey),
  kingdom!="Plantae") |> # remove plants
  group_by(speciesKey,scientificName) |>
  reframe(n=n(),
      exampleurl = references[1]) |> 
  arrange(desc(n)) |>
  head(5) |>
  rowwise() |> 
  mutate(commonname = name_usage(speciesKey, data = "vernacularNames")[[2]] |>
      as.data.frame() |>
      filter(language == "eng") |> 
      pull(vernacularName) |>
      table() |> 
  ...
  select(commonname, image_column, scientificName, n)
...
datatable(
  ...
  options = list(
    columnDefs = list(
      list(targets = which(colnames(test) == "image_column") - 1, 
           orderable = FALSE)
    )
  ),
